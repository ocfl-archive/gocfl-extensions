...
func TestLegacyDirectCleanPathLayout(t *testing.T) {
	...
	runTest := func(name string, config *DirectCleanConfig, cases []testCase) {
		t.Run(name, func(t *testing.T) {
			...
			configPath := path.Join(LegacyDirectCleanName, "config.json")
			...
			cl, ok := genericExtensionManager.(object.ExtensionObjectContentPath)
			assert.True(t, ok, "Extension manager should implement object.ExtensionObjectContentPath interface")
